    ...
    <div className="space-y-3">
      ...
      {items.length === 0 ? (
        <div className="rounded-md border border-dashed p-4 text-center">
          <p className="text-xs text-muted-foreground">{t('ui.goods_editor_intro')}</p>
          <p className="text-[11px] text-muted-foreground mt-1">{t('ui.goods_editor_autocalc')}</p>
        ...
        <div className="space-y-2">
              ...
              <div key={idx} className="rounded-md border bg-card overflow-hidden">
                ...
                <div className="flex items-center gap-2 p-2 bg-muted/30">
                  ...
                  <Input
                    ...
                    placeholder={t('ui.urun_adi_mal_tanimi')}
                  ...
                  <Button
                    ...
                    onClick={() => duplicateItem(idx)} title={t('ui.kalemi_cogalt')}
                  >
                  ...
                  <Button
                    ...
                    onClick={() => removeItem(idx)} title={t('fin.ui.delete_item')}
                  >
                ...
                {isOpen && (
                  <div className="p-3 space-y-3 border-t">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-1">
                          <Input
                            ...
                            placeholder={t('ui.orn_3305_90_00')}
                      ...
                      <div className="space-y-1">
                        <Label className="text-[10px]">{t('ui.mense_ulke')}</Label>
                        <CountryCombobox
                          value={item.origin_country || ''}
                          onChange={(v) => updateItem(idx, { origin_country: v })}
                          placeholder={t('ui.mense_ulke_2')}
                        />
                      </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                      <NumField
                        label={t('ui.kap_adedi')} value={item.quantity}
                        onChange={(v) => updateItem(idx, { quantity: numeric(v) })}
                      />
                      <NumField
                        label={t('ui.brut_agirlik')} suffix="kg" step="0.01" value={item.gross_weight}
                        onChange={(v) => updateItem(idx, { gross_weight: numeric(v) })}
                      />
                      <NumField
                        label={t('ui.net_agirlik')} suffix="kg" step="0.01" value={item.net_weight}
                        onChange={(v) => updateItem(idx, { net_weight: numeric(v) })}
                      />
                      <NumField
                        label={t('vehicle.volume')} suffix="m³" step="0.001" value={item.volume_cbm}
                        onChange={(v) => updateItem(idx, { volume_cbm: numeric(v) })}
                      />
                      <NumField
                        label={t('ui.kiymet')} suffix={currency} step="0.01" value={item.value}
                        onChange={(v) => updateItem(idx, { value: numeric(v) })}
                      />
                    </div>

                    <div className="space-y-1">
                      <Label className="text-[10px]">{t('ui.aciklama_not')}</Label>
                      <Input
                        value={item.note || ''}
          ...
          <div className="rounded-md border bg-primary/5 border-primary/20 p-3">
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3 text-center">
              ...
              <TotalBox label={t('ui.toplam_kiymet')} value={formatNumber(totals.value, 2)} sub={currency} />
            </div>
            <p className="text-[10px] text-muted-foreground text-center mt-2">
              {t('ui.bu_toplamlar_yukaridaki_yuk_bilgileri_alanla')}
            </p>
          </div>
        </div>
      )}
